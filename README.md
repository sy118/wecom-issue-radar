...
- 支持业务配置备份与导入，同时排除本机目录和企业微信凭据、令牌
...
8. 如果启用了 Smart Sheet，默认核对待写入数量后再次确认；只有显式开启自动同步的定时任务会跳过该确认并直接执行外部写入。
...
配置备份不会包含企业微信 Data 目录、导出目录、数据库密钥文件路径，也不会包含 `Corp ID`、`Corp Secret` 或企业微信访问令牌；导入时会继续使用当前电脑上的这些本机配置和凭据。备份会包含模型 API Key、腾讯文档 Webhook 等其他业务凭据，请把备份文件视为敏感文件妥善保管。

# 企微问题雷达
> 本桌面程序参考陈俊宏同学的《AI群咨询日期级自动拉取、识别、登记 WeCom Daily Issue Pipeline v2》思路。
> 再次感谢原作者的开源！

一个面向业务、产品和客户成功团队的 Windows 桌面工具：从本机企业微信群聊中提取指定日期与时间范围内的聊天记录和截图，通过可选的 OCR 与大模型分析整理问题，并导出 Excel、Markdown，或默认在二次确认后同步到腾讯文档 Smart Sheet；符合条件的定时任务也可显式开启自动同步。聊天数据、配置和处理中间结果均保留在本机。
...
- 可创建定时导出任务，配置执行时间、每周执行日、动态日期、多群和处理方式；单个群无记录或失败不会阻断后续群聊
- 本地持久化定时任务执行记录，可分页筛选并查看成功、部分完成、无记录、失败及各群详情
- Smart Sheet 默认采用写入前人工确认；符合条件的定时任务也可显式开启自动同步
- 可配置 SSE、stdio 和 Streamable HTTP MCP 服务，连接测试后按工具 Schema 指纹精确授权
- 可为单个群配置常驻问题监听：合并同一人的连续补充、等待群友回答、并发检索 MCP、独立复核证据，并按安全策略自动发送或进入待发送队列
- 支持业务配置备份与导入，同时排除本机目录、模型密钥、Webhook、MCP 秘密和企业微信凭据、令牌
...
按执行时间、每周执行日和动态日期创建自动导出任务；每个群聊独立保存结果。单个群没有可分析记录或处理失败时，系统会记录该群状态并继续处理后续群聊。页面中的“执行记录”可按任务筛选和分页查看手动、自动执行结果，包括开始/完成时间、整体状态、成功/无记录/失败数量，以及每个群的结果目录或错误原因。当前调度器随桌面应用运行，应用关闭后任务配置仍会保留，但不会在后台自动执行。
...
“MCP 服务”用于登记 SSE、stdio 或 Streamable HTTP 服务并发现工具。监听器授权绑定服务、工具名和 Schema SHA-256；工具结构变化后旧授权立即失效，需要重新确认。

“群监听回复”按群建立监听策略。同一群最多启用一个监听器，不同发送者可并发处理，同一发送者始终串行。系统只处理语义上确认为问题的消息，并先等待群友回答；只有 MCP 返回非空证据且独立模型复核通过，回答才可能发送。自动发送默认关闭，开启前必须发送随机码测试并确认机器人属于所选群。无法解析真实企微艾特、已经有人回答或发送安全门未通过的结果都会进入待发送列表，可人工发送或放弃。
...
- **人工确认（默认）**：手动任务和定时任务都先在本地生成预览。页面会显示目标模板、群聊、文档地址、待写数量和配置校验结果，只有点击“确认写入”后才会向腾讯文档发起请求；也可以暂不同步或放弃待确认结果。写入失败时待确认结果会保留，可查看提示并在核对远端文档后重试。
- **定时任务自动同步（可选）**：默认关闭，仅当定时任务同时启用“大模型分析”和“Smart Sheet”，并已冻结有效的腾讯文档模板 ID 时才能开启。开启后，每个群的预览只要存在待写记录，就会在任务运行期间直接写入腾讯文档，不再生成待人工确认结果。

自动同步属于外部写入操作，启用前还应确认目标模板的 Schema、字段映射和 Webhook 均有效；需要写入图片时，还必须配置有效的企业微信图片上传凭据。自动同步失败不会阻断其他群聊，失败群和错误原因会保存在“定时导出 → 执行记录”中，整体任务会显示为“部分完成”或“失败”，可据此修复配置后重新执行。
...
5. 如需群内智能回复，先在“MCP 服务”连接并发现工具，再到“群监听回复”选择单个群、授权工具并配置等待与并发策略。
6. 在“提示词”中选择或创建业务分析规则，维护对应的问题清单并按需关联腾讯模板。
7. 回到“开始处理”，选择开始/结束日期时间和一个或多个群聊，再选择 OCR、分析和导出方式。结束日期默认今天。
8. 任务结束后直接打开 Excel、Markdown 或完整任务目录。
9. 如果启用了 Smart Sheet，默认核对待写入数量后再次确认；只有显式开启自动同步的定时任务会跳过该确认并直接执行外部写入。
...
- 群聊支持搜索、多选和全选当前搜索结果。系统按选择顺序处理，每个群独立保存目录和结果；单个群无记录时会标记为“无记录”，处理失败时会保留错误并继续下一个群，不会把不同群的数据混入同一个文件。
...
- 定时任务每次完成后会在本地保存执行记录，并按“成功”“部分完成”“无可分析记录”“失败”汇总。记录列表支持分页和按任务筛选，展开后可查看每个群的状态、问题数量、导出目录和错误原因。
- 定时任务启用 Smart Sheet 后默认只生成并持久化待同步预览，不会自动写入腾讯文档。之后可在“定时导出”页核对冻结的模板、群聊、文档地址和待写数量，再明确确认或放弃；只有满足前述启用条件并主动打开“自动同步”的任务才会在执行时直接写入。
...
Python pipeline sidecar
  ...
  ├─ Excel / Markdown 导出
  ├─ Smart Sheet 预览与写入
  └─ 常驻群监听、MCP 检索、证据复核与 durable outbox
```
...
配置备份不会包含企业微信 Data 目录、导出目录、数据库密钥文件路径，也不会包含模型/OCR API Key、腾讯文档或群机器人 Webhook、MCP Headers/Env、`Corp ID`、`Corp Secret` 或企业微信访问令牌；导入时会继续使用当前电脑上已保存的本机配置和凭据。MCP 服务、群监听器及其运行历史首版保存在本机 ReplyRuntime 数据库中，不随便携配置备份迁移。

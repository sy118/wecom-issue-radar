...
Windows 版使用 wxSQLite3 AES-128-CBC 页面加密：16 字节 raw key，每页按
page index + "sAlT" 派生 AES key 和 IV；页面无 SQLCipher HMAC/reserve 区。

由桌面应用“设置 → 企业微信与目录 → 提取密钥”启动（需管理员权限）。
"""
import ctypes
...
# ── 数据库文件收集（原 key_scan_common.collect_db_files）─────────────
...
def _load_config_if_exists():
    config_path = resolve_config_path(allow_example=False)
    try:
...
def _get_db_dir():
    """从用户级/skill 本地 config.local.json 或自动检测获取企微数据目录。"""
    config, _config_path = _load_config_if_exists()
    try:
        db_dir = config.get("wxwork_db_dir") or ""
...
def _get_default_out_file():
    config, config_path = _load_config_if_exists()
    keys_file = config.get("wxwork_keys_file") or "wxwork_keys.json"
    return str(resolve_config_relative_path(keys_file, config_path))


# ── 主流程 ───────────────────────────────────────────────────────────

def main(db_dir: str = None, out_file: str = None):
    """提取企微数据库密钥。

    Args:
        db_dir: 企微 Data 目录。为 None 时自动检测或从 config.json 读取。
        out_file: 输出 JSON 路径。默认项目根目录 wxwork_keys.json。
    """
    if out_file is None:
        out_file = _get_default_out_file()
    ...
    if db_dir is None:
        ...
        if not db_dir or not os.path.isdir(db_dir):
            ...
            if detected:
                ...
                print(f"[*] 请将以下路径写入当前配置文件:")
                print(f"    \"wxwork_db_dir\": \"{detected}\"")
                db_dir = detected
            else:
                print("[!] 未能自动检测企业微信数据目录")
                print("    请在 config.local.json 中设置 wxwork_db_dir")
                return 1
